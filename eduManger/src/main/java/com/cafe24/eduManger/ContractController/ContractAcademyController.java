...
public class ContractAcademyController {
	 ...
	 * @detail 	get방식으로 contractAcademyList url 요청하면 controller단에서
	 * 			contractAcademyList(Model model) 메서드 실행하게되고 service단에서 리턴받은 맵 객체를 Map 클래스 데이터
	 * 			타입으로 선언한 map 변수에 담는다. map 변수에 담겨있던 값들의 주소들을 model 객체에 contractAcademyList라는
	 * 			이름표를 부여해서 담아 contractAcademyList.html 화면에 적은 타임리프 반복문으로 화면에 뿌려준다.
	 */
	 ...
	 * @return 	계약지점목록화면 contract/contractAcademy/contractAcademyList.html
	 * @detail 	get방식으로 contractAcademySearch url 요청하면 controller단에서
	 * 			contractAcademySearch()에 검색조건인 컬럼명, 컬럼내용, 시작기간, 끝기간을 매개변수로 담아 메서드 실행하게되고
	 * 			service단에서 리턴받은 리스트를 model 객체에 contractAcademyList라는 이름표를 부여해서 담아
	...
	public String contractAcademySearch(@RequestParam(value = "cak") String cak,
			@RequestParam(value = "cav") String cav, @RequestParam(value = "cafd") String cafd,
			@RequestParam(value = "cald") String cald, Model model) {
	 ...
	 * @detail 	get방식으로 contractAcademyInsert url 요청하면 controller단에서
	 * 			contractAcademyInsert() 메서드 실행하게 되고 계약 지점 추가 폼이 있는 contractAcademyInsert.html
	 * 			화면으로이동한다.
	 */
	 ...
	 * @detail 	post방식으로 ContractAcademy 타입으로 입력한 데이터가 담긴 객체를 가지고
	 * 			contractAcademyInsert url 요청하면 controller단에서
	 * 			contractAcademyInsert(ContractAcademy contractAcademy) 메서드 실행하게 되고 service단의
	 * 			contractAcademyInsert(contractAcademy) 메서드 실행 요청하고 목록 화면으로 리다이렉트한다.
	 */
	...
	/*
	 * @param 	contractAcademyUpdateById url 계약지점 학원코드 값
	 * @return 	계약지점수정화면 contract/contractAcademy/contractAcademyUpdate.html
	 * @detail 	get방식으로contractAcademyUpdate url 요청할때 controller단에서 각행의 학원코드인 ac_code
	 *			값을 매개변수로 contractAcademyUpdateById()를 실행하게 되고 service단에서
	 * 			contractAcademyUpdateById(ac_code) 실행 요청 후 리턴값을 model객체의 list라는 이름표를 부여하고 담아
	 * 			contractAcademyUpdate.html 화면에서 수정 폼에 value값으로 list에 담긴 값들을 화면에 뿌려준다.
	 */
	 ...
	 * @detail	post방식으로 ContractAcademy 타입으로 입력한 데이터가 담긴 객체를 가지고
	 * 			contractAcademyUpdate url 요청하면 controller단에서
	 * 			contractAcademyUpdate(ContractAcademy contractAcademy) 메서드 실행하게 되고 service단의
	 * 			contractAcademyUpdate(contractAcademy) 메서드 실행 요청하고 목록 화면으로 리다이렉트한다.
	 */
	...
	/*
	 * @param 	contractAcademyDelete url, 학원코드 값
	 * @return 	삭제 처리 후 계약지점 목록화면 redirect:/contractAcademyList
	 * @detail 	삭제버튼을 누르면 get방식으로 각 행의 계약단가 코드값을 받아오면서 모달로 경고창을 띄운다. 모달창에서 YES를 누르면
	 * 			contractAcademyDelete url 요청하면서 미리 받아온 코드값을 매개변수로 받아 contractAcademyDelete() 메서드 실행 요청한다.
	 *			service단에서 매개변수인 ac_code를 전달하여 contractAcademyDelete() 실행하도록 요청하고
	...
	public String contractAcademyDelete(@RequestParam(value = "ac_code") String ac_code) {
		// System.out.println(ac_code + "<-- ac_code ContractAcademyController");
